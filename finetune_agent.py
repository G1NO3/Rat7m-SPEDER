...
class Finetune_Agent():
    ...
    def fit_whole_dataset(self, critic_save_path, u_vec_save_path, times=1000):
        dataset = self.dataset
        agent = self.agent
        args = self.args
        # replay_buffer, state_dim, action_dim, n_task = load_all_keymoseq('test', args.dir, args.device)
        ...
        for k in range(times):
            ...
            self.u_vec = u_vec
            with open(f'./kms/fit_whole_dataset.txt', 'a') as f:
        ...
        return u_vec
    def sample_plot_auc(self, sample_len=250, times=50):
        args = self.args
        dataset = self.dataset
        agent = self.agent
        device = 'cuda:0'
        ...
        for i in range(times):
            ...
            auc_agent, auc_linear = cal_plot_auc(state, action, task, initial_u, 
                            ...
                            save_path=f'figure/{args.env}/{args.alg}/{args.dir}/{args.seed}/auc_longclip_{sample_idx[0]}+{sample_len}.pdf',
        ...
        axis.legend()
        save_fig(f'./kms/auc_longclip_whole_16to512.png')